...
int16_t SPU_GetADPCMSample(uint8_t ch, uint8_t nybble) {
  ...
  int32_t index = channel->adpcmStepIndex;
  if (index < 0) index = 0;
  if (index > 88) index = 88;

  const int32_t step = adpcmStepTable[index];
  int32_t diff = step >> 3;
  if (nybble & 0x1) diff += step >> 2;
  if (nybble & 0x2) diff += step >> 1;
  if (nybble & 0x4) diff += step;
  if (nybble & 0x8) diff = -diff;

  int32_t sample = (int32_t)channel->adpcmLastSample + diff;
  if (sample > 32767) sample = 32767;
  if (sample < -32768) sample = -32768;
  channel->adpcmLastSample = (int16_t)sample;

  index += adpcmIndexTable[nybble & 0xf];
  if (index < 0) index = 0;
  if (index > 88) index = 88;
  channel->adpcmStepIndex = (int16_t)index;
